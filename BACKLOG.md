...
- [~] "Sur safari écran noir même après 20 scd" (screenshot fourni, iPhone, 5G) : même famille de bug que #59 (écran blanc)/#80 (noir puis blanc)/#84 (rose/magenta) — un cas non couvert trouvé dans `sw.js` : quand il n'y a AUCUN shell déjà en cache pour la requête exacte (première visite, ou éviction Safari ITP après 7 jours sans interaction), la navigation attendait le réseau `however long it takes`, sans aucun plafond — une connexion simplement LENTE (pas en échec, donc `fetch()` ne rejette jamais) laissait la promesse de navigation indéfiniment en suspens, exactement "écran noir après 20+ secondes". Ajouté `STALL_TIMEOUT_MS` (8s) + une page de repli minimale inline (message + bouton Réessayer) pour ce cas précis, en plus du `NAV_TIMEOUT_MS` (3s) déjà en place quand un shell EST en cache. Non vérifié sur device réel (pas de Safari/iOS disponible ici) — à confirmer si le symptôme revient.
