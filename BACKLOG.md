...
- [ ] Chaque royaume peut, via un easter egg, débloquer un dieu spécial avec des capacités spéciales. Actives pour le joueur uniquement, sauf en mode le plus dur — ou quand débloqué pour un seigneur, actif qu'on le joue ou qu'il soit adversaire.
  — Scoping fait en direct (questions posées), implémentation reportée à plus tard sur demande explicite ("Plus tard") : (1) une capacité DIFFÉRENTE par royaume (pas un bonus générique recoloré), à choisir en s'inspirant du lore Millon déjà écrit ; (2) déblocage par une vraie condition de jeu atteinte en partie (dominer X tours d'affilée, gagner N combats avec ce royaume...), pas un indice+hasard comme Cthulhu. Reprendre ce chantier avec ces deux contraintes quand la priorité y revient.
