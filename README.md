...
- **Écran de fin de partie et menus (Réglages/Conseils/Le lore) : glisser vers le bas pour révéler la carte** — même principe que pour le replay (glisser toute la page fait réapparaître l'état final du plateau, avec un bouton pour lancer le replay et un bouton pour revenir à l'écran de fin). Et l'écran de fin devrait apparaître d'abord tout en haut de la page (graphique en dessous des boutons), pas nécessiter de remonter pour voir le message.
- **Menus (Réglages/Conseils/Le lore) : une seule surface de défilement**, pas un parchemin fixe avec un texte qui défile à l'intérieur — actuellement ressenti comme "une page dans une page", trop compliqué. Le geste de glisser devrait déplacer le parchemin entier (titre compris), comme dans les Réglages qui ont déjà ce traitement.
