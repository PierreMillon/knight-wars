...
- **Progression cosmétique, pas de pay-to-win.** La monnaie "gueux" et les débllocages narratifs (Lore, Salle des trophées) sont des couches de méta-progression optionnelles ; elles n'ouvrent jamais un mode de triche, seulement du contenu et des bonus obtenus par un vrai exploit en partie. La réserve de gueux (1 gagné par victoire, dépensable pour +1 de force sur une province) est un vrai enjeu, pas un simple compteur qui ne fait que monter : une défaite (conquête ou abandon) la vide entièrement.
...
| **Cérès** | Ne jamais éliminer un seul rival avant le tout dernier instant, puis les éliminer TOUS d'un coup avec une seule attaque **en chaîne** qui laisse aussi le joueur maître de **100% de la carte** (Celtes inclus) | +1 renfort par tour | Plafond de force abaissé à 11 (jamais 12) — plus jamais éligible au pouvoir des 12 (§2.3) |
...
- Une IA au palier de difficulté le plus dur (Templier) ou à Cthulhu bénéficie de tous les bonus des 4 dieux gratuitement, même non gagnés — "elle connaît déjà tous les trucs".
...
- 4 paliers visibles, du plus facile au plus dur : **Gueux** (0), **Page** (0.35), **Écuyer** (0.91), **Templier** (1.3) — la valeur pilote l'agressivité/force de départ de l'IA. Renommés une première fois (chaque nom descend d'un rang social : l'ancien "Seigneur", le plus dur, disparaît), puis le plus dur renommé une seconde fois de "Chevalier" à "Templier" ("je veux que le chevalier à pied soit chevalier et celui à cheval templier" — son icône est le chevalier monté ; aucun palier "Chevalier" à pied n'existe encore, le nom reste libre) — les valeurs elles-mêmes sont inchangées dans les deux cas.
...
- 4 victoires par palier de difficulté (Gueux/Page/Écuyer/Templier)
...
- **Regroupement villages/capitale par province** (voir §2.6) : plusieurs hexagones fusionnés en une seule province, un seul affichant le chiffre de force ("la capitale"), avec des troupes qu'on pourrait stationner sur n'importe quel village de la province indépendamment de ce chiffre. Reposé et clarifié en direct ("juste une règle de trajet", pas ce regroupement) — la partie "chemin le plus court" de l'idée d'origine est déjà garantie par le code (voir §2.6), seul ce regroupement lui-même reste non conçu/non implémenté ; nécessiterait de repenser le modèle territoire/hexagone actuel.
